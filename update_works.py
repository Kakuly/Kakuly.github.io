...
def get_tags_from_ai(title, description):
    ...
    if model:
        ...
        4. もしネット上に情報がない場合のみ、概要欄の文脈から「Kakuly」の直後にある役割を拾ってください。
    ...
    # 2. バックアップ判定（AI失敗時：Kakulyの名前がある行だけを凝視する）
    tags = []
    lines = (title + "\n" + description).split('\n')
    for line in lines:
        l_lower = line.lower()
...
.video-item h3 { font-family: 'Montserrat', 'Noto Sans JP', sans-serif !important; font-size: 0.85rem !important; height: 3em; overflow: hidden; margin-bottom: 10px !important; line-height: 1.3; }
